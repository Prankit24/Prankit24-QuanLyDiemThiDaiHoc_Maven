...
public class ManagerView extends javax.swing.JFrame {
    ...
    private Chart chart1 = new Chart();  // Khởi tạo chart1 trực tiếp
...
>>>>>>> b156958 (View fix)
        ...
        jPanel4Layout.setHorizontalGroup(
            ...
            .addComponent(btnStatisticType, javax.swing.GroupLayout.Alignment.TRAILING, javax.swing.GroupLayout.DEFAULT_SIZE, javax.swing.GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE)
        );
        ...
        setTitle("Quản lý các đối tượng đặc biệt ");
...
>>>>>>> b156958 (View fix)
    ...
    for (int i = 0; i < size; i++) {
        ...
        specialPersons[i][1] = list.get(i).getName();             // Name
        specialPersons[i][2] = fDate.format(list.get(i).getBirthday()); // Birthday
        specialPersons[i][3] = list.get(i).getAddress();          // Address
        specialPersons[i][4] = fDate.format(list.get(i).getOpeningDate());  // Opening Date
        specialPersons[i][5] = list.get(i).getType();          // Type (CMT)
        specialPersons[i][6] = list.get(i).getImage();            // Image
    }
...
public void showCountListSpecialPersons(List<SpecialPerson> list) {
    int size = list.size();
    FieldSum.setText(String.valueOf(size)); // Hiển thị số lượng trong FieldSum
}

// Trong ManagerView.java
public void showSpecialPerson(SpecialPerson specialPerson) {
    // Hiển thị thông tin của SpecialPerson vào các trường trong giao diện
    FieldID.setText(String.valueOf(specialPerson.getId()));
    FieldName.setText(specialPerson.getName());
    // Tiếp tục điền các thông tin khác vào các trường cần thiết trong ManagerView
}
// Trong ManagerView.java
public SpecialPerson getSpecialPersonInfo() {
    ...
    /**
     * điền thông tin của hàng được chọn từ bảng specialPerson vào các trường
     * tương ứng của specialPerson.
     */
...
public Student getStudentInfo() {
    ...
    try {
        Student specialPerson = new Student();
        if (FieldID.getText() != null && !"".equals(FieldID.getText())) {
            specialPerson.setId(Integer.parseInt(FieldID.getText()));
        }
        specialPerson.setName(capitalizeWords(FieldName.getText().trim()));
        ...
        // Set the type of ID card (CMT)
        specialPerson.setTypeCMT(ComboBoxType.getSelectedItem().toString().trim());

        // Set image as byte array
        specialPerson.setImage(specialPerson_image); // assuming specialPerson_image is byte[]
    ...
    public void showStatisticTypeSpecialPersons(List<Student> list) {
    ...
    public void showStatisticAgeSpecialPersons(List<Student> list) {
   ...
   public void addAddSpecialPersonListener(ActionListener listener) {
    btnAdd.addActionListener(listener);  // Kết nối nút Thêm với sự kiện
}

public void addEditSpecialPersonListener(ActionListener listener) {
    btnEdit.addActionListener(listener); // Kết nối nút Chỉnh sửa với sự kiện
}

public void addDeleteSpecialPersonListener(ActionListener listener) {
    btnDelete.addActionListener(listener); // Kết nối nút Xóa với sự kiện
}

public void addClearListener(ActionListener listener) {
    btnClear.addActionListener(listener); // Kết nối nút Làm mới với sự kiện
}

public void addSearchListener(ActionListener listener) {
    btnSearch.addActionListener(listener); // Kết nối nút Tìm kiếm với sự kiện
}

public void addSortByIDListener(ActionListener listener) {
    btnSortByID.addActionListener(listener); // Kết nối nút Sắp xếp theo ID với sự kiện
}

public void addSortByNameListener(ActionListener listener) {
    btnSortByName.addActionListener(listener); // Kết nối nút Sắp xếp theo tên với sự kiện
}

public void addSortByOpeningDateListener(ActionListener listener) {
    btnSortByOpeningDate.addActionListener(listener); // Kết nối nút Sắp xếp theo ngày mở với sự kiện
}

public void addSortByYearListener(ActionListener listener) {
    btnSortByYear.addActionListener(listener); // Kết nối nút Sắp xếp theo năm với sự kiện
}

public void addStatisticListener(ActionListener listener) {
    btnStatistic.addActionListener(listener); // Kết nối nút Thống kê với sự kiện
}

public void addStatisticAgeListener(ActionListener listener) {
    btnStatisticAge.addActionListener(listener); // Kết nối nút Thống kê độ tuổi với sự kiện
}

public void addStatisticTypeListener(ActionListener listener) {
    btnStatisticType.addActionListener(listener); // Kết nối nút Thống kê loại đối tượng với sự kiện
}

public void addStatisticUnderListener(ActionListener listener) {
    btnStatisticUnder.addActionListener(listener); // Kết nối nút Thống kê khác với sự kiện
}

public void addSearchDialogListener(ActionListener listener) {
    btnSearchDialog.addActionListener(listener); // Đăng ký sự kiện cho nút tìm kiếm trong hộp thoại
}
...
// Các sự kiện cho checkbox và combobox
public void addCheckBoxListener(ActionListener listener) {
    CheckBoxAddress.addActionListener(listener);  // Kết nối checkbox Address với sự kiện
    CheckBoxName.addActionListener(listener); // Kết nối checkbox Name với sự kiện
    CheckBoxYear.addActionListener(listener); // Kết nối checkbox Year với sự kiện
}

public void addComboBoxListener(ActionListener listener) {
    ComboBoxType.addActionListener(listener); // Kết nối combobox Type với sự kiện
}

// Các sự kiện cho các nút khác như: btnSearchDialog, btnCancelSearch...
public void addCancelSearchSpecialPersonListener(ActionListener listener) {
    btnCancelSearch.addActionListener(listener); // Kết nối nút hủy tìm kiếm với sự kiện
}

public void addCancelDialogListener(ActionListener listener) {
    btnCancelDialog1.addActionListener(listener); // Kết nối nút hủy dialog tìm kiếm với sự kiện
}

public void addUndoListener(ActionListener listener) {
    btnManagerUndo.addActionListener(listener); // Kết nối nút hoàn tác với sự kiện
}

public void addImageSpecialPersonListener(ActionListener listener) {
    btnImage.addActionListener(listener); // Kết nối nút ảnh với sự kiện
}
