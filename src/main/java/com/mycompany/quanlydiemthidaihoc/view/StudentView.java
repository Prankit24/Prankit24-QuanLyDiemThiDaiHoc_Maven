...
public class StudentView extends javax.swing.JFrame {
    ...
    private void initComponents() {
        ...
        jLabel10.setText("<html>Quản lý điểm thi học sinh<br> ");
    ...
    public Student getStudentInfo() {
        if (!validateName() || !validateBirthday() || !validateSex()) {
            return null;  // Kiểm tra hợp lệ trước khi trả về thông tin sinh viên
        }

        try {
            Student student = new Student();
            student.setId(Integer.parseInt(FieldID.getText().trim()));  // Lấy ID sinh viên
            student.setName(FieldName.getText().trim());
            student.setBirthday(BirthdayChooser.getDate());  // Ngày sinh
            student.setAddress(TextAreaAddress.getText().trim());  // Địa chỉ
            student.setGender(CheckBoxMale.isSelected() ? "Male" : "Female");  // Giới tính
            return student;
        } catch (Exception e) {
    ...
    public void showCountListStudents(List<Student> list) {
        int size = list.size();
        // Giả sử có một JTextField hoặc JLabel để hiển thị số lượng
        FieldSum.setText(String.valueOf(size));
    }

    public void showStatisticTypeCMT(List<Student> list) {
        Map<String, Integer> countMap = new HashMap<>();
        for (Student student : list) {
            String typeCMT = student.getTypeCMT();  // Assuming `getTypeCMT()` returns the type of ID (e.g., CMND, CCCD, or Định danh)
            countMap.put(typeCMT, countMap.getOrDefault(typeCMT, 0) + 1);
        }

        // Hiển thị kết quả thống kê
        FieldSumCMND.setText(String.valueOf(countMap.getOrDefault("CMND", 0)));
        FieldSumCCCD.setText(String.valueOf(countMap.getOrDefault("CCCD", 0)));
        FieldSumDD.setText(String.valueOf(countMap.getOrDefault("Định danh", 0)));
    }

    public void showStatisticIDCard(List<Student> list) {
        Set<String> idFamilySet = new HashSet<>();
        for (Student student : list) {
            idFamilySet.add(student.getIdCard());  // Assuming `getIdCard()` returns the ID card number or family ID
        }
        FieldSumFamily.setText(String.valueOf(idFamilySet.size()));  // Hiển thị số lượng hộ gia đình (ID gia đình độc nhất)
    }
    ...
    public void showListStudents(List<Student> list) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            residents[i][1] = list.get(i).getIdCard();
    ...
    public void showStudents(Student st) {
        // Sử dụng đối tượng 'st' thay vì 'resident'
        FieldIDFamily.setText("" + st.getIdCard());
        FieldName.setText(st.getName());

        // Chuyển đổi ngày sinh thành chuỗi và gán vào BirthdayChooser
        if (st.getBirthday() != null) {
            // Gán ngày sinh vào BirthdayChooser (JDateChooser)
            BirthdayChooser.setDate(st.getBirthday());

            // Nếu bạn muốn hiển thị ngày sinh dưới dạng chuỗi trong FieldBirthPlace, chuyển Date thành String
            SimpleDateFormat dateFormat = new SimpleDateFormat("dd/MM/yyyy");
            FieldBirthPlace.setText(dateFormat.format(st.getBirthday()));
        }
        ...
        } else if ("Nữ".equals(st.getGender())) {
        ...
        // Cập nhật loại CMT và số CMT
        ComboBoxCMT.setSelectedItem(st.getTypeCMT());
        FieldCMT.setText(st.getCMT());

        // Kích hoạt các nút Sửa và Xóa, tắt nút Thêm
    ...
    public void fillStudentFromSelectedRow(List<Student> list) throws ParseException {
        // Lấy chỉ số của hàng được chọn 
        int row = tableResident.getSelectedRow();
        if (row >= 0) {
            // Lấy ID của resident từ cột đầu tiên
            int residentID = Integer.parseInt(tableResident.getModel().getValueAt(row, 0).toString());
            // Tìm kiếm resident trong danh sách dựa trên ID
            Student selectedResident = findResidentByID(list, residentID);

            if (selectedResident != null) {
                // Tạo SimpleDateFormat để chuyển đổi ngày thành chuỗi
                SimpleDateFormat dateFormat = new SimpleDateFormat("dd/MM/yyyy");

                // Điền thông tin vào các trường hợp hợp lệ
                FieldID.setText(String.valueOf(selectedResident.getId()));
                FieldIDFamily.setText(selectedResident.getIdCard());
                ...
                if (selectedResident.getBirthday() != null) {
                    FieldBirthPlace.setText(dateFormat.format(selectedResident.getBirthday()));
                }

                // Kiểm tra và cập nhật giới tính
                if ("Nam".equals(selectedResident.getGender())) {
                    CheckBoxMale.setSelected(true);
                    CheckBoxFemale.setSelected(false);
                } else if ("Nữ".equals(selectedResident.getGender())) {
                    CheckBoxMale.setSelected(false);
                    CheckBoxFemale.setSelected(true);
                } else {
                    CheckBoxMale.setSelected(false);
                ...
                showMessage("Không tìm thấy cư dân với ID: " + residentID);
            }
        } else {
            showMessage("Vui lòng chọn một hàng trong bảng.");
        }
    }
    ...
    private Student findResidentByID(List<Student> StudentList, int StudentID) {
    ...
    public void showCountListStudent(List<Student> list) {
        int size = list.size();
        FieldSum.setText(String.valueOf(size));
    }
    ...
    public int getChooseSelectSort() {
        if (CheckBoxSortID.isSelected()) {
            return 1;
        } else if (CheckBoxSortName.isSelected()) {
            return 2;
        } else if (CheckBoxSortIDFamily.isSelected()) {
            return 3;
        }
        return 0;
    }

    public void showStatisticTypeCMTUpdated(List<Student> list) {
        Map<String, Integer> countMap = new HashMap<>();
    ...
    public int getChooseSelectSearch() {
        if (CheckBoxIDFamily.isSelected()) {
            return 1;
    ...
    public void addSortStudentListener(ActionListener listener) {
    ...
    public void addCancelSearchStudentListener(ActionListener listener) {
